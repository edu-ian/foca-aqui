...
import { Sun, Moon, Flame, LogOut, Sparkles, Menu } from 'lucide-react';
  ...
  return (
    <header className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 p-5 rounded-2xl bg-brand-card border border-brand-border transition-all shadow-lg hover:shadow-xl duration-300">
      <div className="flex items-center gap-3 w-full sm:w-auto">
        {/*  Caixas e botões (Hamburger Menus) contidos nas porções ou regiões Esquerdas. Próximos ao card / Cartão de perfil do Usurio. */}
        <motion.button
        ...
        >
          <img src="/logo.png" alt="Foca Aqui" className="w-10 h-10 object-contain" />
        </motion.div>
        <div>
          <h1 className="text-base font-medium text-brand-text/60 font-mono tracking-tight">FOCO ATIVO</h1>
      ...
      <div className="flex items-center gap-3 shrink-0">
        {/* Mini-Tag Ousados / Placas ou Insignias do tipo Badges ("Chamas / Fogos Quentes / Fogo"). Focadas nas marcações (Streak / Ofensivas em Dias). */}
        <motion.div
          whileHover={{ scale: 1.05 }}
        ...
        {/* Interruptores Gráficos / Ou Botões ativados p/ comutar e engajar configurações e Temas estéticos (Claros VS Escuros!). */}
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        ...
        {/* Botão Encerrador Visual ("Sai/Logout/Finaliza Sessão Autenticada"). (Saira do App) */}
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => {
